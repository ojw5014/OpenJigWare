...
namespace OpenJigWare.Docking
{
    public partial class frmKinematics : Form
    {
        ...
        private void MakeDHSkeleton(bool bVisible, float fSize, Color cColor, String strDh)
        {
            ...
            MakeDHSkeleton(bVisible, fSize, cColor, strDh, nStartGroupNumber,
                fOffset_X, fOffset_Y, fOffset_Z);
        }
        private void MakeDHSkeleton(bool bVisible, float fSize, Color cColor, string strString, int nStartGroupNumber, float fOffset_X, float fOffset_Y, float fOffset_Z)
        {
            MakeDHSkeleton(bVisible, fSize, cColor, strString, nStartGroupNumber, fOffset_X, fOffset_Y, fOffset_Z, 0.0f, 0.0f, 0.0f);
        }
        private void MakeDHSkeleton(bool bVisible, float fSize, Color cColor, string strString, int nStartGroupNumber, float fOffset_X, float fOffset_Y, float fOffset_Z, float fOffset_Pan, float fOffset_Tilt, float fOffset_Swing)
        {
        ...
        private void Axis_Select(int nIndex)
        {
            if (nIndex < m_C3d.m_CHeader.pSMotorInfo.Length)
            {
                ...
                txtAxis_Reserve_Int_3.Text = Ojw.CConvert.IntToStr(m_C3d.m_CHeader.pSMotorInfo[nIndex].nReserve_3);
        ...
        private void txtAxis_Reserve_Int_3_TextChanged(object sender, EventArgs e)
        {
            int nIndex = (lstboxMotorIndex.SelectedIndex < 0) ? 0 : lstboxMotorIndex.SelectedIndex;
            m_C3d.m_CHeader.pSMotorInfo[nIndex].nReserve_3 = Ojw.CConvert.StrToInt(((TextBox)sender).Text);
            DisplayIndex(nIndex);
        }
